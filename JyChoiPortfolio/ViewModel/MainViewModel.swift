...
class MainViewModel: CommonViewModel {
    ...
    private func requestBookList(_ text: String) {
        ...
        self.anyCancelation = self.api.requestBookList(text: text, target: self.target, sortingOption: self.sortingOption, pageNo: self.pageModel.currentPageNo, size: PageDataModel.pageSize).receive(on: DispatchQueue.main).sink(receiveCompletion: {complete in
