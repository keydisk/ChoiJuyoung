            ...
            VStack(spacing: 0) {
                ...
                ScrollView(.vertical) {
                    ...
                    HStack {
                        Spacer()
                        Text("소개")
//                            .foregroundColor(self.selectMenu == .introduce ? Color.black : Color.gray)
                            .modifier(TextStyleModifier(isBlack: true)) // 텍스트 뷰 모디파이어~
                            .font(.spoqaRegular(fontSize: 20)).onTapGesture {
                            self.selectMetaInfo(.introduce)
                        }
                        Spacer()
                        Text("저자").modifier(TextStyleModifier(isBlack: false)).onTapGesture {
                            self.selectMetaInfo(.author)
                        }
                        Spacer()
                        Text("출판사명").modifier(TextStyleModifier(isBlack: false)).onTapGesture {
                        ...
                        ScrollView(.horizontal, content: {
                            ...
                            HStack {
                                ForEach(self.viewModel.sameAuthList, content: { model in
                                    ...
                                    BookMetaInfoView(model: model, viewModel: self.viewModel, sendView: $moveBookDetail).onTapGesture {
        ...
        }.onReceive(self.viewModel.sendDataUpperView, perform: { model in
